...
- Use the repository's existing phase-document convention. If none exists, use `docs/phases/<phase-id>/` with `README.md`, `STATUS.md`, and `STEP_<NNN>_<slug>.md`.
- Treat the current `STEP_*.md` as the sole authority for file scope and side-effect policy. Let `STATUS.md` point to that specification instead of restating its boundary.
...
2. Use `absent`, `scaffolded`, `implemented`, `tested`, or `accepted` for capability state. Use `not-started`, `in-progress`, `development-complete`, `accepted`, `release-ready`, or `release-blocked` for phase state.
...
After writing or updating artifacts, compute the current step checkpoint with `python <skill-dir>/scripts/validate_phase_artifacts.py --print-checkpoint <step-path>`, record it in `STATUS.md`, then run `python <skill-dir>/scripts/validate_phase_artifacts.py <phase-dir>`. Do not hand off artifacts that fail validation.
...
Include the phase `README.md` only when the current step requires dependency context that is not already captured in `STATUS.md` and the step specification.
...
3. verify that no forbidden external effect occurred;
4. review contracts, migrations, privacy, rollback, and failure behavior in proportion to risk;
5. update `STATUS.md` with actual commands, results, risks, file scope, and Git checkpoint only after verification;
6. detail the next step from the new repository state, refresh its SHA-256 checkpoint in `STATUS.md`, and run the artifact validator.
